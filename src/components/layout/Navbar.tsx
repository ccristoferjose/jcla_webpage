...
import { getDictionary, pageKeyFromPathname, path, type Locale, type PageKey } from "@/i18n";
...
const NAV_PAGES: Array<{ key: PageKey; label: keyof ReturnType<typeof getDictionary>["nav"] }> = [
  { key: "home", label: "home" },
...
export function Navbar({ locale }: { locale: Locale }) {
  ...
  return (
    <header
      className={cn(
        "sticky top-0 z-50 h-[var(--header-height)] bg-cream-100/85 backdrop-blur-md transition-shadow duration-300",
    ...
    >
      ...
      {/* Menú móvil: panel completo bajo el header. */}
      <div
        className={cn(
          "fixed inset-x-0 top-[var(--header-height)] bottom-0 z-40 bg-cream-100 lg:hidden",
      ...
      >
        <nav aria-label={dict.nav.menu} className="container-page flex flex-col py-6">
          {NAV_PAGES.map(({ key, label }) => (
            <Link
              key={key}
              href={path(key, locale)}
